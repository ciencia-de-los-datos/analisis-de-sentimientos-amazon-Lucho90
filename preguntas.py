...
def pregunta_01():
    ...
    df_tagged = df[df["Nota"].notnull()]
    df_untagged = df[df["Nota"].isnull()]

    x_tagged = df_tagged["Texto"]
    y_tagged = df_tagged["Nota"]

    x_untagged = df_untagged["Texto"]
    y_untagged = df_untagged["Nota"]
